  ...
  const syncState = async () => {
    try {
      const [stateData, leaderboardData] = await Promise.all([
        ...
        fetchJson<{ clubs: Array<{ name: string; score: number }> }>("/api/leaderboard"),
      ]);
    ...
    <div className="projector-shell">
      ...
      {countdownRemaining !== null && countdownRemaining > 0 && (
        <div className="countdown-overlay">
          <div className="countdown-number" style={{ fontSize: "12rem" }}>
            {countdownRemaining}
          </div>
      ...
      <div className="projector-grid">
              ...
              <div className="projector-options-grid">
                {(["A", "B", "C", "D"] as const).map((key) => {
                  ...
                  return (
                    <div
                      key={key}
                      className={`projector-option-box ${isCorrect ? "correct" : ""}`}
                    >
                      <div
        ...
        {/* Right Column: Giant Live Scoreboards */}
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          {/* STACK.PUSH SCORE */}
          <div
            className="score-card stack"
            style={{
              padding: "28px 24px",
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div className="score-card-title" style={{ fontSize: "1.1rem" }}>
                ⚡ STACK.PUSH
              </div>
              {isStackLeading && (
                <span style={{ fontSize: "0.85rem", fontWeight: 900, color: "#fbbf24", background: "rgba(245, 158, 11, 0.2)", padding: "4px 10px", borderRadius: "6px" }}>
                  ★ LEADER
                </span>
              )}
            </div>
            <div className="score-card-points" style={{ fontSize: "4.5rem", lineHeight: 1.1, marginTop: "10px" }}>
          ...
          {/* IT INNOVATORS SCORE */}
          <div
            className="score-card innovators"
            style={{
              padding: "28px 24px",
          ...
          <div className="glass-card" style={{ padding: "20px", textAlign: "center" }}>
            ...
            <div style={{ fontSize: "1.2rem", fontWeight: 800, marginTop: "6px", color: isStackLeading ? "#60a5fa" : isInnovatorsLeading ? "#34d399" : "#fbbf24" }}>
              {isStackLeading
